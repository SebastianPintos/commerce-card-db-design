...
* _consumoValidoTest()_ : Agrega un consumo válido y comprueba que se haya agregado a la tabla compras.
* _consumoTarjetaInvalidaTest()_ : Agrega un consumo en el que la tarjeta es inválida. Comprueba que se haya agregado el rechazo y la alerta con código 0 (correspondiente a alertas por rechazo). Retorna true si la comprobación es exitosa, false en caso contrario.
* _ consumoCodSeguridadInvalidoTest()_ : Agrega un consumo en el que el código de seguridad es inválido. Comprueba que se haya agregado el rechazo y la alerta con código 0 (correspondiente a alertas por rechazo). Retorna true si la comprobación es exitosa, false en caso contrario.
* _consumoExcedeLimiteTest()_ : Agrega un consumo que excede el límite de la tarjeta indicada. Comprueba que se haya agregado el rechazo y la alerta con código 0 (correspondiente a alertas por rechazo). Retorna true si la comprobación es exitosa, false en caso contrario.
* _consumoTarjetaExpiradaTest()_ : Agrega un consumo en el que la tarjeta está expirada. Comprueba que se haya agregado el rechazo y la alerta con código 0 (correspondiente a alertas por rechazo). Retorna true si la comprobación es exitosa, false en caso contrario.
* _consumoTarjetaSuspendidaTest()_ : Agrega un consumo en el que la tarjeta está suspendida. Comprueba que se haya agregado el rechazo y la alerta con código 0 (correspondiente a alertas por rechazo). Retorna true si la comprobación es exitosa, false en caso contrario.
* _consumoAlerta1Test()_ : Agrega dos consumos de una misma tarjeta en comercios distintos ubicados en el mismo código postal, donde la tarjeta es válida y el monto es suficiente. Comprueba que se haya agregado la alerta con código 1 correspondiente a compras con esas características realizadas en un lapso menor a un minuto. Retorna true si la comprobación es exitosa, false en caso contrario.
* _consumoAlerta5Test()_ : Agrega dos consumos de una misma tarjeta en comercios, donde la tarjeta es válida y el monto es suficiente. Comprueba que se haya agregado la alerta con código 5 correspondiente a compras con esas características en un lapso menor a 5 minutos. Retorna true si la comprobación es exitosa, false en caso contrario.
* _consumoAlerta32Test()_ : Agrega dos consumos de una misma tarjeta los cuales exceden los límites de la misma. Esto, genera dos rechazos por exceso de límite de tarjeta.  Comprueba que se haya agregado la alerta con código 32 asociada a tarjetas que tienen dos rechazos por exceso de límite en un mismo día. Retorna true si la comprobación es exitosa, false en caso contrario.
...
* _TestearConsumo()_ : Llama a los métodos encargados de crear las _stored procedures_ para realizar el test. Los mismos son: _consumoValidoTest_, _consumoTarjetaInvalidaTest_, _consumoCodSeguridadInvalidoTest_, _consumoExcedeLimiteTest_, _consumoTarjetaExpiradaTest_, _consumoTarjetaSuspendidaTest_, _consumoAlerta1Test_, _consumoAlerta5Test_  y _consumoAlerta32Test_.
